...
//   This month: budget.php sends computed dates
//   This year : budget.php sends computed dates
...
// ============================================================
$sql        = "SELECT b.id, b.amount, b.remarks, b.date_created,
                      c.category_name
               FROM budgets b
               JOIN categories c ON b.category_id = c.id
               WHERE b.user_id = ?
                 AND c.category_type = 'Sales'";
$bindTypes  = "i";
$bindValues = [$userId];

if ($startDate !== '' && $endDate !== '') {
    $sql        .= " AND DATE(b.date_created) BETWEEN ? AND ?";
    $bindTypes  .= "ss";
    $bindValues[] = $startDate;
    $bindValues[] = $endDate;
} elseif ($startDate !== '') {
    $sql        .= " AND DATE(b.date_created) >= ?";
    $bindTypes  .= "s";
    $bindValues[] = $startDate;
} elseif ($endDate !== '') {
    $sql        .= " AND DATE(b.date_created) <= ?";
...
// ============================================================
if ($isCsv) {
    // Build a safe filename from the range
    $safeName = 'income_' . ($startDate ?: 'all') . ($endDate ? '_to_' . $endDate : '') . '.csv';
    ...
    // Meta row
    fputcsv($out, ['BizTracker Income Report', $rangeLabel, 'Generated: ' . date('d M Y H:i')]);
    fputcsv($out, []); // blank row

    // Header row
    fputcsv($out, ['No', 'Category', 'Amount (RM)', 'Remarks', 'Date']);
...
<head>
    ...
    <title>Income Report | BizTracker</title>
...
<body class="bg-white p-10">
    ...
    <div class="text-center mb-8">
        <h1 class="text-2xl font-bold">Income Report</h1>
    ...
    <?php if (empty($rows)): ?>
        <p class="text-center text-gray-500 py-12">No income records found for this date range.</p>
    <?php else: ?>
